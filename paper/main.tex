        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Embodied Attention Budgeting: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot foundation model efficiency. The motivating bet is: Allocate perception and action attention by physical consequence rather than token salience.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For robot foundation model efficiency, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
augmentation\_as\_data\_wam & nominal & 0.650 & 0.350 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.258 & 0.742 \\
...
\end{tabular}
        ...
        \section{Conclusion}
        Embodied Attention Budgeting argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
